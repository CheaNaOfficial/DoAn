...
const getList = async (req, res) => {
    try {
        ...
        var sql = "SELECT e.*, r.Name as RoleName FROM user e "+
...
try {
    ...
    // Check Tel is exist ?
    const [user] = await db.query("SELECT * FROM user WHERE Tel=:Tel",{Tel:Tel});
    if(user.length == 0){
        res.json({
            error : true,
    ...
    const [data] = await db.query("UPDATE user SET Password=:Password WHERE Tel = :Tel",{Password:passHash,Tel:Tel});
    res.json({
        message : data.affectedRows ? "Password set successfully" : "Somthing wrong!"
    })

} catch (err) {
    logError("user.setPassword", err, res)
}
...
const login = async (req, res) => {
    try {
      ...
      const [user] = await db.query("SELECT * FROM user WHERE Tel=:Tel", {Tel:Username });
